 ...
 */
const CARD_BASE =
  'rounded-3xl border border-taupe-200/80 bg-surface shadow-[0_4px_20px_-4px_rgba(26,15,82,0.05)] transition-all duration-300 hover:shadow-[0_12px_24px_-4px_rgba(26,15,82,0.08)]';

/** Alternated per "How I work" card so the row of three isn't monochrome. */
const INDEX_TONES = [
  // Fixed pastel chip backgrounds (plum-50/magenta-50) pair with fixed
  // brand-color text so they read the same "sticker" way in both themes.
  'bg-plum-50 text-plum-700',
  'bg-magenta-50 text-magenta-700',
  'bg-taupe-100 text-muted',
];
    ...
    <main className={PAGE_SHELL}>
      ...
      <div className={`grid grid-cols-12 gap-5 lg:gap-6 ${SECTION_HEADER_TO_CONTENT}`}>
        ...
        <div className={`col-span-12 flex flex-col justify-between p-6 lg:col-span-7 lg:p-8 ${CARD_BASE}`}>
          <p className="text-xl font-medium leading-snug text-foreground lg:text-2xl">
            {tHero.rich('bio', {
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <span className="rounded-full bg-taupe-100 px-3 py-1 text-xs font-semibold text-muted">
              {tPerson('jobTitle')}
            </span>
            <span className="text-sm text-muted/70">{tPerson('yearsExperience')}</span>
          </div>
        </div>
