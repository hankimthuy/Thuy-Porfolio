    ...
    return (
        <a href={pdfUrl} target="_blank" rel="noreferrer" className="block">
            <div className={`relative ${className ?? ''}`}>
                ...
                {status === 'loading' && (
                    <div className="absolute inset-0 flex items-center justify-center text-sm text-[#424874]/70 bg-white/60">
                        Loading preview...
                    </div>
                )}

                {status === 'error' && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-sm text-[#424874]/70 px-4 text-center bg-white/80">
                        <div>Unable to preview PDF</div>
                        {errorMessage && <div className="text-xs text-[#424874]/60">{errorMessage}</div>}
                    </div>
                )}
            </div>
        </a>
