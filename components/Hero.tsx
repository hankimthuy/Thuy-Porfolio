    ...
    <section id="about" className="max-w-7xl mx-auto px-6 lg:px-6 pt-24 lg:pt-[9rem] pb-12 lg:pb-16 flex flex-col lg:flex-row items-center gap-8 lg:gap-16 bg-white">
      ...
      <div className="flex-1 space-y-4 lg:space-y-6 z-10 text-center lg:text-left relative">
        ...
        <div className="space-y-3 lg:space-y-4 relative">
          <h1 className="font-semibold text-[#424874] tracking-tight">
            <span className="block text-3xl lg:text-6xl leading-[1.15] text-balance">
              Hi! I&apos;m {PERSON.brandName},
            </span>

            <span className="mt-3 lg:mt-4 flex flex-wrap items-baseline justify-center lg:justify-start gap-x-2.5 gap-y-1">
              {PERSON.jobTitles.map((title, index) => (
                <span key={title} className="inline-flex items-baseline gap-x-2.5">
                  {index > 0 && (
                    <span aria-hidden="true" className="text-[#A6B1E1] text-xl lg:text-2xl font-light leading-none">
                      ·
                    </span>
                  )}
                  <span className="text-xl sm:text-2xl lg:text-[2.25rem] leading-tight font-semibold">
                    {title}
                  </span>
                </span>
              ))}
            ...
            <span className="block text-base lg:text-xl mt-3 lg:mt-4 font-normal opacity-70">
      ...
      <div className="flex-1 relative w-full flex justify-center items-center py-6 lg:py-10">
        ...
        <div className="relative z-10 w-[240px] lg:w-[380px] group transition-transform duration-500 hover:scale-[1.01]">
          ...
          <Image
            ...
            alt={`${PERSON.brandName} — ${PERSON.jobTitle}, professional portrait`}
            width={380}
            height={480}
            priority
