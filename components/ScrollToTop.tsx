'use client';

import { useState, useEffect } from 'react';
import { LuChevronUp } from 'react-icons/lu';
import { useTranslations } from 'next-intl';

export default function ScrollToTop() {
  const t = useTranslations('common');
  const [isVisible, setIsVisible] = useState<boolean>(false);

  const toggleVisibility = (): void => {
    if (window.scrollY > 300) {
      setIsVisible(true);
    } else {
      setIsVisible(false);
    }
  };

  const scrollToTop = (): void => {
    window.scrollTo({
      top: 0,
      behavior: 'smooth',
    });
  };

  useEffect(() => {
    window.addEventListener('scroll', toggleVisibility);

    return () => {
      window.removeEventListener('scroll', toggleVisibility);
    };
  }, []);

  return (
    <div className="fixed bottom-6 right-6 z-50 sm:bottom-8 sm:right-8">
      <button
        type="button"
        onClick={scrollToTop}
        aria-label={t('scrollToTop')}
        aria-hidden={!isVisible}
        className={`flex h-11 w-11 items-center justify-center rounded-full border border-ocean-200 bg-surface/95 text-ocean-700 shadow-bento backdrop-blur-sm transition-all duration-300 hover:border-ocean-500 hover:bg-ocean-100 hover:text-ocean-900 hover:shadow-bento-hover active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-ocean-500 focus-visible:ring-offset-2 ${
          isVisible
            ? 'pointer-events-auto translate-y-0 opacity-100'
            : 'pointer-events-none translate-y-3 opacity-0'
        }`}
      >
        <LuChevronUp className="h-5 w-5" strokeWidth={2.25} aria-hidden />
      </button>
    </div>
  );
}
